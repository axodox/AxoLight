...
namespace AxoLight::Colors
{
  ...
  const float _maxLightness = 0.75;
  ...
  void enhance(std::vector<rgb>& colors)
  {
    ...
    {
      /*rgb corrected;
      corrected.r = min(255.0, max(0.0, 0.8005918860435486 * color.r + 0.23673559725284576 * color.g + 0.054503846913576126 * color.b));
      corrected.g = min(255.0, max(0.0, 0.018977604806423187 * color.r + 0.9645366072654724 * color.g + 0.06882637739181519 * color.b));
      corrected.b = min(255.0, max(0.0, 0.0500728040933609 * color.r + -0.060099828988313675 * color.g + 1.0020443201065063 * color.b));
      color = corrected;*/
